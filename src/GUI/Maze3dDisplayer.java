		...
		addPaintListener(new PaintListener() 
		{
			...
			public void paintControl(PaintEvent e) 
			{
				if (m_mazeData != null) 
				{
					...
					int width = m_mazeData[0].length;
					int height = m_mazeData.length;
					...
					{
						double w0 = 0.7 * cellWidth + 0.3 * cellWidth * i / m_mazeData.length;
						double w1 = 0.7 * cellWidth + 0.3 * cellWidth * (i + 1) / m_mazeData.length;
						double start = mx - w0 * m_mazeData[i].length / 2;
						double start1 = mx - w1 * m_mazeData[i].length / 2;
